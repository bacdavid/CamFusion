function [centers, misc] = detectCentersROI(frame, rois, objectDict, nInstances, minDistance, cameraParameters, surfThreshold, nOctaves, ambigRatio)
%DEPRECATED!
%DETECTCENTERROI Detect objects in frames. Misc contains a bunch of stuff
%which could be used for visualization (not implemented).
% inputs:
%   frame               Frame from camera 1.
%   rois                Cell of ROIs for camera 1.
...
%                       frame.
    ...
    %% Detect center in ROI
    [features, points, frame] = detectFeaturesROI(frame, rois, cameraParameters, surfThreshold, nOctaves);
    [matchIdx, matchIds, validIds] = matchDictionary(features, objectDict, ambigRatio);
    
    centers = []; imageIds = []; objectIds = [];
